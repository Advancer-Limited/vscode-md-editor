...
- [x] Sidebar restructured to a tab bar (client-rendered — VS Code's view-container API has no native horizontal-tab primitive, only stacked accordion views or one webview per container) with the existing Markdown Links view as tab 1
- [x] New Mermaid tab: lists every `.mmd`/`.mermaid` file in the workspace via a new lightweight `MermaidFileIndexService` (filenames only, no wikilink/backlink parsing needed) — search + click to open, no one-click template insert or diagram thumbnails (deliberately out of scope for this pass)
- [x] Flat/folder view toggle, independent per tab (own button + own persisted state via `vscode.setState()`), always-fully-expanded folder tree (no collapse state to track) built client-side from each node's `relativePath` — folders only exist in the tree when a file needs them, so empty folders are pruned by construction, not by a filter pass
- [x] Typing in a tab's search box forces that tab's view back to flat (folder view doesn't compose with search); clearing the search reverts to the toggle's last state; the toggle button disables itself while a query is active
- [x] Right-click a folder header (folder view only) opens a floating context menu — mirrors the existing wikilink-picker/mermaid-template-menu floating-div pattern, since webviews don't get contributed context menus for arbitrary DOM elements: "Add Markdown"/"Add Mermaid" (host-side `showInputBox`, since webview `prompt()` is blocked — same restriction as `confirm()`/`print()`) creates and opens the file; "Show in Explorer" runs the built-in `revealInExplorer` command
- [x] Verified with 27 new Playwright checks (tab switching, mermaid list + open, folder-tree grouping/pruning/nesting, search-forces-flat + toggle disabling, per-tab state independence, both folder-menu actions for both file kinds, outside-click dismissal, setState persistence) plus regression checks confirming the pre-existing Markdown Links behavior (active-file highlight, backlink expand/collapse, link-click-to-open, Show Graph button) survived the graph.js rewrite intact
- [x] Version bumped 1.1.2 -> 1.2.0 (minor: new user-facing feature, no breaking changes)

## 2026-07-23 Sidebar polish (flat-list simplification, tab rename/icons, header) + Markdown Print + toolbar icons (v1.3.0)

- [x] Diagnosed a user report of "duplicate" files in the sidebar's flat view: verified via a Fable sub-agent that this was genuinely different files (different service folders sharing a filename + immediate parent folder name, e.g. two different `.../docs/00-overview.md`), not a real duplicate-entry bug — confirmed by tracing the old `getFolder()`'s immediate-parent-only computation against the exact repeat count/pattern in the screenshot
- [x] Fable's investigation also surfaced a real, separate bug while checking: `getRelativePath()` in both `FileIndexService` and `MermaidFileIndexService` (plus `extension.ts`'s `getActiveFilePath`) used a case-sensitive `startsWith()`/`slice()` to compute a workspace-relative path — a document opened via a differently-cased path (reachable even on Windows, mid-path casing, not just the drive letter, which VS Code's URIs do normalize) could silently produce a bogus second index entry rather than updating the existing one. Replaced with Node's `path.relative()` (empirically verified case-insensitive on Windows for this) in all three places
- [x] Per user steer: flat view no longer shows a folder tag at all (plain alphabetical list) rather than showing more folder path — folder view (the toggle) is where that context lives
- [x] Sidebar tabs: "Markdown Links" renamed to "Markdown", both tabs given a small file-type icon left-aligned before their label, tab content left-aligned instead of centered
- [x] Sidebar panel header set to "MD & MMD Editor" via `webviewView.title` (a documented, writable runtime property — verified via the actual `@types/vscode` type definitions rather than guessing at package.json's `viewsContainers`/`views` title-concatenation behavior)
- [x] Investigated adding an Advancer icon next to the panel header text — not supported: `WebviewView.title` is a plain string with no icon slot, and there's no other public contribution point for it; the activity-bar rail icon (already the Advancer mark) is the only icon slot VS Code exposes for this container
- [x] Added a Print button to the main Markdown WYSIWYG editor's toolbar, mirroring the Mermaid editor's existing Print feature (opens a standalone page in the real browser since `window.print()` is blocked in VS Code's sandboxed webview) — extracted the print-file sweep/TTL housekeeping shared by both editors into `src/printFiles.ts` rather than duplicating it a second time; the print page embeds the editor's own stylesheet so headings/tables/code blocks/embedded Mermaid diagrams render the same as the live preview
